...
typedef struct {
  ...
  uint8_t networkSpeed; /* The max network speed in Mbs , set to 0x00 or 0xff to
                           use link speed */
  ...
  double salinity;     /* THe salinity to be used with internal speed of sound
                          calculations (ppt) */
  // "To enable the 32-bit mode, you can OR-in 0x00000200 into the `extFlags`
  // field.""
  uint32_t extFlags;
  uint32_t reserved[8];
...
public:
  ...
  double rangeResolution; /* The range in metres corresponding to a single range
                             line */
...
public:
  ...
  uint32_t firmwareVersion1; /* The arm1 firmware version major(8 bits), minor(8
                                bits), build (16 bits) */
...
// If the part number is not in this list, it defaults to the
// "partNumberUndefined" configuration
